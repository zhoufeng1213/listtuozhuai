package com.btzh.fly.listtuozhuai.drag_list_view;
...
import com.btzh.fly.listtuozhuai.utils.CommonTool;
...
//                    if (!mIsScrolling && !mLongClickFlag && mDownX - mMoveX >= DELETE_X_MIN && Math.abs(mMoveY - mDownY) <= DELETE_Y_SLOP) {
                    if (!mIsScrolling && !mLongClickFlag && Math.abs(mMoveY - mDownY) <= DELETE_Y_SLOP) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mIsDraging) {
                    ...
                    if(myDropListener != null){
                        myDropListener.onDrop(mAdapter.getDataList());
                    }
                } else if (mIsDeleting) {
                    // 停止删除中
//                    stopDeleting();
...
//            mAdapter.swapData(mFromPosition, mToPosition);
    ...
    private MyDropListener myDropListener;
    public void setDropListener(MyDropListener myDropListener){
        this.myDropListener = myDropListener;
    }

    public interface  MyDropListener{
        void onDrop(List<HashMap<String,Object>> data);
    }
}
